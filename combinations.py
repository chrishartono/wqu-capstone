...
def SearchForGoodCombinations(prices_df: pd.DataFrame, all_possible_combinations: list, comovement_type: ComovementType, n_jobs: int,
	...
	# results = parallel(delayed(TestCombinationComovement)(*p) for p in params)
	# results = parallel(delayed(TestCombinationComovement)(*p) for p in tqdm(params, total=len(params), desc="Combinations search:"))
	...
	all_good_combinations = [(comb, coint_vector, mi_score) for isGood, comb, coint_vector, mi_score in results if isGood and coint_vector is not None]
	all_good_combinations.sort(key=lambda tup: tup[2], reverse=True)
	good_combinations = [(comb, coint_vector) for comb, coint_vector, mi_score in all_good_combinations[:num_good_combs_to_choose]]
